...
cmd_n1(args) {
    if(isDefined(self.cooldown)) { message_player("Cooldown, wait " + 
        self.cooldown + " seconds."); return;
    }
    messages = [];
...
cmd_maplist(args) { 
    ...
    color[2] = "4";
    color[3] = "6";
    color = codam\_mm_mmm::array_shuffle(color);
    color = color[0];
    for(i = 0; i < gametypes.size; i++) {
        ...
        for(m = 0; m < maps[gametype].size; m++) {
            message += maps[gametype][m]; if((m + 1) % 7 == 0) { message_player("^" + color + message);
                message = "";
            } else
                message += " ";
        }
...
cmd_fps(args) {
    ...
    args1 = args[1]; // name if(!isDefined(args1)) { message_player("^1ERROR: ^7Invalid argument."); return;}
    if(args.size > 2) {
        for(a = 2; a < args.size; a++)
            if(isDefined(args[a])) args1 += " " + args[a];
    }
